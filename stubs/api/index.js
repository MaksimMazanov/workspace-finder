...
router.get('/search', (req, res) => {
  ...
  if (type === 'name') {
    ...
    results = workplaces.filter(workplace =>
      workplace.employeeName.toLowerCase().includes(q.toLowerCase()) &&
...
router.get('/workplaces', (req, res) => {
  ...
  // Группируем места по блокам
  const blocksMap = {};
  workplaces.forEach(workplace => {
    const blockCode = workplace.blockCode;
    ...
    blocksMap[blockCode].places.push(workplace);
    blocksMap[blockCode].total++;
    if (workplace.status === 'occupied') {
      blocksMap[blockCode].occupied++;
    }
  });
